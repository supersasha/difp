...
def couplers(datasheet):
    ...
    red = np.array(p['red']['dye']['data'])
    green = np.array(p['green']['dye']['data'])
    blue = np.array(p['blue']['dye']['data'])
    def solve(basic, coupler1, coupler2):
        def f(x):
            vs = [k*basic + (1-k)*(x[0]*coupler1 + x[1]*coupler2)
...
def profile_cmd(opts):
    ...
    for k in ks:
        for idx in [0, 1, 2]:
            ds[idx].append(density(idx, status, dye, 40, k, ys[idx]))
    print('ks: ', ks)
    print('ds: ', ds)
    ...
    def solve_k(idx, x):
        def f(k):
            j = datasheet.curve(idx, x)-mc[idx]+d0[idx] - density(idx, status, dye, q[idx], k, ys[idx])
            return j*j
        bounds = [(0, 1)]
    ...
    check(new_profile, status)

def check(profile, status):
    plt.subplot(111)
    for idx in range(3):
